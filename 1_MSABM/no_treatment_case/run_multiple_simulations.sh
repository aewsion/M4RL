...
do  
  ...
  do
      ...
      current_jobs=$(ps aux | grep -E "no_treatment_case" | grep -v grep | wc -l) # current number of 'no_treatment_case' jobs
  ...
  # start no_treatment_case jobs
  nohup ./bin/no_treatment_case no_treatment_case_output $set 1 200 1 &

  # ./bin/no_treatment_case \          # path to the executable file for MSABM no treatment case
  # no_treatment_case_output \         # output folder name to store the population results (argv[1])
