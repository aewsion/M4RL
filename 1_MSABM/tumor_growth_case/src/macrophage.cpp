...
void Macrophage::migration(CellGrids &cg, Diffusibles &diff){
    ...
    probs[0] = (1 - cg.allcells[i+ix[0]][j+jx[0]]) * (k*D_M/pow(h,2) - k*a_MC*(C[2]-C[0])/(4*pow(h,2))); // move up
    probs[1] = (1 - cg.allcells[i+ix[1]][j+jx[1]]) * (k*D_M/pow(h,2) - k*a_MC*(C[3]-C[1])/(4*pow(h,2))); // move right
    probs[2] = (1 - cg.allcells[i+ix[2]][j+jx[2]]) * (k*D_M/pow(h,2) - k*a_MC*(C[0]-C[2])/(4*pow(h,2))); // move down
    probs[3] = (1 - cg.allcells[i+ix[3]][j+jx[3]]) * (k*D_M/pow(h,2) - k*a_MC*(C[1]-C[3])/(4*pow(h,2))); // move left
