...
class Environment{
public:
    Environment(double stepSize, std::string folder, std::string st_folder, int set, double M0RecProb, double TCProl,
                double k_M12, double kd_M12, double k_I, double p_tc, double p_d, int pha, int rand, 
                double a_c, double a_i, double a_erk, int grid_size); // create TME
...
private:
    ...
    void load(); // load virtual TME from ST data-based csv files
    ...
    // arrays used for ST data-based virtual TME loading
