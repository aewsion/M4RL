...
#include <math.h>
#include <ctime>
Diffusibles::Diffusibles(double DX, int size) {
    // representation, unit, references of parameters
    // are listed in S2 Table Cytokines part & Drugs part
    gs = size;
    dx = DX;
    dt = 5; // specific for ST data spots
    dt_drug = 5; // specific for ST data spots
    time = 0;
    ...
    D_CSF1R_I = 2e-10*(dx/0.0015)*(dx/0.0015); // specific for ST data spots
    D_IGF1R_I = 2e-10*(dx/0.0015)*(dx/0.0015);
    D_EGFR_I = 2e-10*(dx/0.0015)*(dx/0.0015); // if necessary
    ...
    drug_time = 0;
    drug_feed_time = (0)*24*60*60; // set different combination treatment later in Diffusibles::diffusion
...
void Diffusibles::diffusion(CellGrids &cg, double tstep) {
    ...
    for (int q = 0; q < t; q++) {
        ...
        for(int i = 1; i < gs-1; i++){
            CSF1[i][0] = CSF1[i][2]; // Neumann boundary condition
            CSF1[i][gs-1] = CSF1[i][gs-3];
            EGF[i][0] = EGF[i][2];
            EGF[i][gs-1] = EGF[i][gs-3];
        }
        for(int j = 1; j < gs-1; j++){
            CSF1[0][j] = CSF1[2][j];
            CSF1[gs-1][j] = CSF1[gs-3][j];
            EGF[0][j] = EGF[2][j];
            EGF[gs-1][j] = EGF[gs-3][j];
        }
    ...
    for (int q = 0; q < t_drug; q++) {
        // set different combination treatment here
